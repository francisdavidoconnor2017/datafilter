	...
	transform = () => {
		...
		for (let key in users) {
			...
			for (let propt in obj) {
				...
				if (propt === 'gender' && gender !== 'Both') {
					if (this.state.genderValue !== obj['gender']) {
						genderTrip = true;
						continue;
					}
				}
				if (propt === 'region' && region !== 'All') {
				...
				if (propt === 'birthday' && !genderTrip && !regionTrip) {
					if (
						obj['spend'] <= this.state.slideMax &&
						obj['spend'] >= this.state.slideMin
					) {
		...
		monthlyspend.reduce(function(a, b, i) {
		...
		return (
			<div className="App">
				...
				<CenterDiv>
					<StyledSlider
						range={true}
						defaultValue={[this.state.slideMin, this.state.slideMax]}
						step={this.state.fullStep}
						min={this.state.fullMin}
						max={this.state.fullMax}
						marks={this.marks}
